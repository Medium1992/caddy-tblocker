...
| `header` | нет | Задаёт заголовок на ответе блокировки. Повторяй опцию для нескольких заголовков, повторяй имя для нескольких значений, а `-<имя>` удаляет заголовок — включая дефолтный `Cache-Control: no-store`. |

Запрос, у которого `{client_ip}` пуст или не парсится, пропускается, а не блокируется.

**Подгонка под маскировку.** По умолчанию бан отвечает голым `403`, а это отличимый ответ: клиент понимает, что его забанили, и на сайте-обманке это ломает легенду. `handle_response` внутри `reverse_proxy` тут не поможет — он переписывает только ответы апстрима, а бан до прокси вообще не доходит. Связка `status` и `header` делает ответ блокировки неотличимым от того, что сайт отдаёт обычно:
